...
func (r *usersRepository) LoginUser(email string, password string) (*users.User, errors.RestErr) {
	...
	if response == nil || response.Response == nil {
		return nil, errors.NewInternalServerError("invalid restclient response when trying to login user", errors2.New("restclient error"))
	}
	...
	if err := json.Unmarshal(response.Bytes(), &user); err != nil {
		return nil, errors.NewInternalServerError("error when trying to unmarshal users login response", errors2.New("restclient error"))
	}
	return &user, nil
}
